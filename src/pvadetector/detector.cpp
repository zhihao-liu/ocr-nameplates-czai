...
std::vector<Detection> Detector::detect(cv::Mat const& img, std::string class_mask) const
{
...
vector<Detection> Detector::overThresh(int* keep, int num_out, float* sorted_pred_cls, float CONF_THRESH, string className) const
{
	vector<Detection> dets;
	int i = 0;
	while (i < num_out)
	{
		...
		det.setRect(Rect(Point(sorted_pred_cls[keep[i] * 5 + 0], sorted_pred_cls[keep[i] * 5 + 1]), Point(sorted_pred_cls[keep[i] * 5 + 2], sorted_pred_cls[keep[i] * 5 + 3])));
