...
void PvaDetector::setComputeMode(std::string const& mode, int id) {
    if (mode == "gpu") {
        Caffe::SetDevice(id);
...
std::vector<Detection> PvaDetector::detect(cv::Mat const& img) const {
    ...
    bbox_delt = m_net->blob_by_name(
            "bbox_pred")->cpu_data();    // bbox_delt is offset ratio of bounding box, get by bounding box regression
...
std::vector<Detection> PvaDetector::detect(cv::Mat const& img, std::string const& class_mask) const {
    std::vector<Detection> dets = detect(img);
    std::vector<Detection> mask_dets;
    for (auto const& det: dets) {
        if (det.getClass() == class_mask) {
            mask_dets.push_back(det);
        }
    }
    if (!mask_dets.empty())
...
std::vector<Detection> PvaDetector::detect(cv::Mat const& img, std::set<std::string> const& class_mask_set) const {
    std::vector<Detection> dets = detect(img);
    std::vector<Detection> mask_dets;
    for (auto const& det: dets) {
        if (class_mask_set.count(det.getClass()) == 1) {
            mask_dets.push_back(det);
        }
    }
    if (!mask_dets.empty())
...
void PvaDetector::drawBox(cv::Mat& img, std::vector<Detection> const& dets, int Trans_x, int Trans_y) {
    for (auto const& det: dets) {
        Rect old_rect = det.getRect();
        ...
        os << det.getScore();
        std::string showTxt = det.getClass(); //+ "," + os.str();;
        putText(img, showTxt, // Mat, text
                cv::Point(det.getRect().x + Trans_x, det.getRect().y + Trans_y - 2), //oriPt
...
void PvaDetector::drawBox(cv::Mat& img, std::vector<Detection> const& dets) {
    for (auto const& det: dets) {
        rectangle(img, det.getRect(), cv::Scalar(255, 0, 255), 1);
        std::ostringstream os;
        os.precision(2); //保留2位有效数字
        os << det.getScore();
        std::string showTxt = det.getClass(); //+ "," + os.str();;
        putText(img, showTxt, // Mat, text
                cv::Point(det.getRect().x, det.getRect().y - 2), //oriPt
                cv::FONT_HERSHEY_SIMPLEX, 1, //Font
                cv::Scalar(0, 255, 255), 1); // color, linear width
//        putText(img, showTxt, // Mat, text
//            cv::Point(dets[i].getRect().x, dets[i].getRect().y - 2), //oriPt
...
void PvaDetector::drawBox(cv::Mat& img, std::vector<Detection> const& dets, string const& type) {
    for (auto const& det: dets) {
        std::ostringstream os;
        os.precision(2); //保留2位有效数字
        os << det.getScore();
        ...
        if (type == showTxt) {
            putText(img, "warning!", // Mat, text
                    ...
                    cv::FONT_HERSHEY_SIMPLEX, 1, //Font
                    cv::Scalar(0, 255, 255), 1); // color, linear width
            rectangle(img, det.getRect(), cv::Scalar(0, 0, 255), 2);
        } else {
            putText(img, showTxt, // Mat, text
                    cv::Point(det.getRect().x, det.getRect().y - 2), //oriPt
                    cv::FONT_HERSHEY_SIMPLEX, 1, //Font
                    cv::Scalar(0, 255, 255), 1); // color, linear width
            rectangle(img, det.getRect(), cv::Scalar(0, 255, 0), 2);
        }
    }
}
...
* =====================================================================================
*/
void PvaDetector::bbox_transform_inv(int num, const float* box_deltas, const float* pred_cls, float* boxes, float* pred,
    ...
    for (int i = 0; i < num; i++) {
        ...
        ctr_x = float(boxes[i * 4 + 0] + 0.5 * width);        // (ctr_x, ctr_y) is center coordinater of bounding box
        ...
        for (int j = 0; j < class_num; j++) {
            ...
            pred[(j * num + i) * 5 + 0] = float((std::max)((std::min)(pred_ctr_x - 0.5 * pred_w, (img_width - 1) * 1.0), 0.0));    // avoid over boundary
            pred[(j * num + i) * 5 + 1] = float((std::max)((std::min)(pred_ctr_y - 0.5 * pred_h, (img_height - 1) * 1.0), 0.0));
            pred[(j * num + i) * 5 + 2] = float((std::max)((std::min)(pred_ctr_x + 0.5 * pred_w, (img_width - 1) * 1.0), 0.0));
            pred[(j * num + i) * 5 + 3] = float((std::max)((std::min)(pred_ctr_y + 0.5 * pred_h, (img_height - 1) * 1.0), 0.0));
...
vector<Detection> PvaDetector::overThresh(int* keep, int num_out, float* sorted_pred_cls, float CONF_THRESH, string const& className) {
    vector<Detection> dets;
    int i = 0;
    while (i < num_out) {
        ...
        det.setRect(Rect(Point(int(sorted_pred_cls[keep[i] * 5 + 0]), int(sorted_pred_cls[keep[i] * 5 + 1])),
                         Point(int(sorted_pred_cls[keep[i] * 5 + 2]), int(sorted_pred_cls[keep[i] * 5 + 3]))));
