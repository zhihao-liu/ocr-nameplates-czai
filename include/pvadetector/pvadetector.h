...
public:
    ...
    void init(std::string const& net_pt, std::string const& net_weights, std::vector<std::string> const& classes);
    ...
    std::vector<Detection> detect(cv::Mat const& img, std::set<std::string> const& class_mask_set) const;
    //显示函数
    static void drawBox(cv::Mat& img, std::vector<Detection> const& dets);
    static void drawBox(cv::Mat& img, std::vector<Detection> const& dets, std::string const& type); // added by myf; edited by lzh
    static void drawBox(cv::Mat& img, std::vector<Detection> const& dets, int Trans_x, int Trans_y); // added by myf; edited by lzh
    static std::vector<Detection> overThresh(int* keep, int num_out, float* sorted_pred_cls, float CONF_THRESH, std::string const& className);
